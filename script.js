...
// Removed rotation smoothing variables
...
async function switchCamera() {
    ...
    // Clear smoothed position to prevent sudden jump after camera switch
    smoothedPos = { x: 0, y: 0, z: 0 };
...
function setupThreeJS(videoWidth, videoHeight) {
    ...
    // Video Texture Mirroring
    if (CAMERA_MODE === 'user') {
        videoTexture.wrapS = THREE.RepeatWrapping;
        videoTexture.offset.x = 1;
        videoTexture.repeat.x = -1; // Mirror horizontally
    } else {
        videoTexture.wrapS = THREE.ClampToEdgeWrapping;
        videoTexture.offset.x = 0;
        videoTexture.repeat.x = 1; // No mirror
    }
        ...
        try {
            ...
            if (poses.length > 0) {
                ...
                if (leftShoulder && rightShoulder && leftShoulder.score > 0.4 && rightShoulder.score > 0.4) {
                    ...
                    // const bodyAngle = Math.atan2(rightShoulder.y - leftShoulder.y, rightShoulder.x - leftShoulder.x); // NO ROTATION
                    
                    if (wingsAsset && isSplatDataReady) {
                        positionSingleWingSet(wingsAsset, spineCenter); // Removed bodyAngle argument
...
/**
 * Position and Scale the wings asset based on the spine center.
 * Rotation is FIXED.
 */
function positionSingleWingSet(wing, spineCenter) {
    ...
    // FIX 2: Apply FIXED Rotation (No body-based rotation)
    const targetRotX = 0; 
    const targetRotY = Math.PI; // 180 degrees around Y-axis to face forward
    const targetRotZ = 0;      // No rotation around Z-axis (no shoulder tilt)
    
    // Setting rotation directly (or with smoothing if desired, but direct is simpler)
    wing.rotation.set(targetRotX, targetRotY, targetRotZ);
    // Since we removed smoothedRot, we set directly for simplicity.
}
